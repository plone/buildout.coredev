...
por ejemplo el archivo :file:`plip-1234-widget-frobbing.cfg`. Es archivo sera definido en el
branches/trunks que usted esta trabajando con su PLIP. Esto debería lucir algo 
así como lo siguiente:

En el archivo :file:`plips/plip-1234-widget-frobbing.cfg`::
...
Trabajando en un PLIP
^^^^^^^^^^^^^^^^^^^^^

Para trabajar en un PLIP, usted ejecute el bootstrap y entonces indíquele a buildout para utilizar
este configuración plip::

  bin/buildout -c plips/plip-1234-widget-frobbing.cfg

Si estás utilizando un archivo :file:`local.cfg` para extender su archivo plip con algunos
cambia que no quieres hacer commits accidentalmente, se consciente que 
necesitas a sobre-escribir algunas configuraciones del archivo :file:`plipbase.cfg` para evitar creados algunos
archivos en el directorio :file:`plips` o en el directorio por encima del
buildout directorio.  Como este::
...
Este debería ser llamado :file:`plip_<number>_notes.txt`. Este debe incluir (pero no es limitado a solo eso):
